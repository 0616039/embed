...
int main()
{
    ...
    while(1){
        int mode = 0;
        cout << "Choose mode 1: detect, 2: performance, 3: end" << endl;
        cin >> mode;
        if(mode == 1){
            while(1){
                ...
                vector<Rect> faces;
                faceCascade.detectMultiScale(grayscale, faces, 1.2, 3, CASCADE_DO_ROUGH_SEARCH, Size(50,50));
		    ...
		    // detect eyes
                    ...
                    if(predict == 1 and confidence <= 100){
                        string name = "Ivy";
                        putText(frame, name, text_lb, FONT_HERSHEY_COMPLEX, 1, Scalar(0, 0, 255));
                    }
                    else if(predict == 2 and confidence <= 100){
                        string name = "Tiffany";
                        putText(frame, name, text_lb, FONT_HERSHEY_COMPLEX, 1, Scalar(0, 0, 255));
                    }
                    else{
        ...
        // performance
		    ...
		    // detect eyes
                    ...
                    cout << predict << " " << confidence << endl;
                    double elapsed_time;
                    if(predict == 1 && confidence <= 100){
                        end = clock();
                        elapsed_time = (double)(end-start) / CLOCKS_PER_SEC * 1000.0;
                        string name = "Ivy(" + to_string(elapsed_time)+")";
                        putText(frame, name, text_lb, FONT_HERSHEY_COMPLEX, 1, Scalar(0, 0, 255));
                        cout << "Ivy:" << to_string(elapsed_time) << "ms" << endl;
                        end_flg = true;
                    }
                    else if(predict == 2 && confidence <= 100){
                        end = clock();
                        elapsed_time = (double)(end-start) / CLOCKS_PER_SEC * 1000.0;
                        string name = "Tiffany(" + to_string(elapsed_time)+")";
                        putText(frame, name, text_lb, FONT_HERSHEY_COMPLEX, 1, Scalar(0, 0, 255));
                        cout << "Tiffany:" << to_string(elapsed_time) << "ms" << endl;
                        end_flg = true;
                    }
                }
